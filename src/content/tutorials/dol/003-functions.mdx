...
Add methods inside gen declarations:

```dol
pub gen MyceliumNode {
  has id: u64
  has position: Vec3
  has connections<Vec<Ui64>
...
pub fun multiply(x: i64, y: i64) -> i64 {
  val result = x * y
...
pub fun find_nearby_nodes(pos: Vec3, max_dist: f64) -> List<Ui64> {
  ...
  for (id, node) in this.nodes {
    val dx = pos.x - node.position.x
    val dy = pos.y - node.position.y
    val dz = pos.z - node.position.z
    val dist = (dx * dx + dy * dy + dz * dz).sqrt()
    if dist <= max_dist {
      result.push(id)
    }
  }
...
```dol
pub fun apply_twice(f: (i64) -> i64, x: i64) -> i64 {
  f(f(x))
}
...
pub system MyceliumNetwork {
  ...
  pub fun from_spore(position: Vec3) -> MyceliumNetwork {
    val initial_tip = HyphalTip {
      position: position,
      direction: Vec3 { x: 0.0, y: 0.0, z: 1.0 },
      age: 0.0
    }

    val initial_node = MyceliumNode {
      id: 0,
      position: position,
      tips: [initial_tip],
      connections: []
