...
// Access by index
let first = items[0]        // 1
let last = items[items.len() - 1]  // 3

// Length
let count = items.len()     // 3

// Add elements (requires mut)
items.push(4)               // [1, 2, 3, 4]

// Check membership
let has_two = items.contains(2)  // true
```
...
```dol
// Create a map
let mut scores: Map<String, Int64> = Map.new()
...
// From the codebase
let call_args = args.map(|arg| CallArg {
  name: None,
  value: arg,
  span: span.clone()
})
...
fun has_enough_branching_points() -> Bool {
  let branch_count = this.nodes.values()
...
```dol
let numbers = [1, 2, 3, 4, 5]
let sum = numbers.reduce(0, |acc, n| acc + n)  // 15

// From the codebase
fun calculate_biomass() -> Float64 {
  let node_mass = sum(this.nodes.values().map(|n| n.stored_nutrients.total_mass()))
  let edge_mass = sum(this.edges.map(|e| e.length() * e.diameter * 0.1))
...
```dol
let numbers = [1, 2, 3, 4, 5]
let first_even = numbers.find(|n| n % 2 == 0)  // Some(2)
let first_big = numbers.find(|n| n > 10)        // None
```
...
let has_even = numbers.any(|n| n % 2 == 0)  // true
let all_positive = numbers.all(|n| n > 0)   // true
let all_even = numbers.all(|n| n % 2 == 0)  // false
```
