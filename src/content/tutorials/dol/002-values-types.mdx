...
Here's a typed gene from the DOL codebase:
...
- `pub` makes the gene publicly visible
...
exegesis {
  A Field represents a named component of a gene with an
...
pub gene Point {
  ...
  fun distance_to(other: Self) -> Float64 {
...
- Use generics with type parameters: `gene Name<T> { ... }`
- Default values: `has field: Type = default`
- Recursive types use `Box<T>`

## Practice Exercise

Create a typed gene for a "blog post" with:
- `id: UInt64`
- `title: String`
- `body: String`
- `status: enum { Draft, Published, Archived }`
- `tags: List<String>`
- `published_at: Option<UInt64>`
