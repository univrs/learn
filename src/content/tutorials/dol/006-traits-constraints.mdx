...
| Keyword | Meaning |
|---------|---------|
| `matches` | Value must equal declared specification |
| `never` | Action is prohibited |
...
constraint resource.allocation {
  ...
  allocated never exceeds maximum
  minimum never exceeds allocated
}
...
constraint identity.cryptographic {
  ...
  public_key derives from keypair
...
exegesis {
  ...
  is generated once and never modified.
}
```
...
- Use `matches` for equality: `value matches declared`
- Use `never` for prohibitions: `property never changes`
