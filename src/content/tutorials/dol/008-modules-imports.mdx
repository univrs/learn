...
Every DOL file can declare its module with a version:
...
pub gene GeneDecl {
  ...
  exegesis {
    A gene declaration with type parameters and fields.
...
exegesis {
  ...
  The service uses the hello.world gene for basic message structure.
...
evolves container.lifecycle @0.0.2 from @0.0.1 {
  ...
  // Changed type
  change timeout: Int32 to timeout: Int64
}
...
```dol
module myproject.internal @ 1.0.0
...
pub(super) gene ParentVisible {
  has value: Int64
}

// Public within package
pub(crate) gene PackageVisible {
  has config: Config
}
...
module myproject.user @ 1.0.0
module myproject.auth @ 1.0.0
module myproject.session @ 1.0.0

// Avoid: catch-all modules
module myproject.misc @ 1.0.0  // Too vague
```
...
module myproject @ 2.0.0  // Incompatible API changes

// Minor: New features
module myproject @ 1.1.0  // Added new genes/functions

// Patch: Bug fixes
module myproject @ 1.0.1  // Fixed behavior, no API changes
```

## Key Takeaways

- Declare modules with `module path.name @ version`
...
// In blog/types.dol:
module blog.types @ 1.0.0

pub gene Post {
  has id: UInt64
  has title: String
  has content: String
  has author_id: UInt64
  has published_at: Option<UInt64>
