...
objectives:
  ...
  - "Understand gen syntax for compatibility"
tags:
  - "dol"
  - "type"
  - "trait"
...
fun distance(val a: Point2D, val b: Point2D) -> f64 {
    val dx = b.x - a.x
    val dy = b.y - a.y
    (dx * dx + dy * dy).sqrt()
}
...
pub trait Measurable {
    fun distance_from_origin(val self) -> f64
}

/// Calculate distance between two points
pub fun distance(val a: Point2D, val b: Point2D) -> f64 {
    val dx = b.x - a.x
    val dy = b.y - a.y
    (dx * dx + dy * dy).sqrt()
}
