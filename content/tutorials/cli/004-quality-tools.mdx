...
20 | gen Another {
   | ^^^^
   |
   = Missing 'exegesis' block in gen 'Broken'
...
$ vudo check --format json src/broken.dol
{
  ...
  "errors": [
    ...
    },
    {
      "line": 18,
      "column": 1,
      "message": "Missing 'exegesis' block in gen 'Broken'",
      "severity": "error",
      "code": "E042"
    }
  ],
...
VUDO generates documentation from `docs` blocks:
...
45 | pub gen Config {
   |          ^^^^^^
   |
   = Public gen 'Config' has minimal exegesis.
