...
-- A type implementing Collection<UInt64>
pub type IdSet uses Collection<UInt64> {
    ids: Set<UInt64>
}
```
...
trait ResourceManaged {
    get_cpu_usage: fun() -> Float64
    get_memory_usage: fun() -> UInt64
    set_cpu_limit: fun(limit: Float64) -> Void
    set_memory_limit: fun(limit: UInt64) -> Void
}
...
trait PagedRepository<T, ID> extends Repository<T, ID> {
    find_page: fun(page: UInt64, size: UInt64) -> Page<T>
    count: fun() -> UInt64
}
...
trait Notifiable {
    ...
    get_unread_count: fun() -> UInt64
    mark_as_read: fun(notification_id: UInt64) -> Void
}
...
trait NotificationChannel extends Notifiable, NotificationSender {
    ...
    get_delivery_status: fun(notification_id: UInt64) -> DeliveryStatus
}
